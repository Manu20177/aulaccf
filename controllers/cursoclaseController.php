<?php
	...
	class cursoclaseController extends cursoclaseModel{
		...
		public function obtenerclases_controller($id_alumno,$code){
			$query = self::connect()->prepare("SELECT * FROM `curso_alumno` ca LEFT JOIN curso_clase cc on cc.id_curso=ca.id_curso LEFT JOIN clase c on c.id=cc.id_clase WHERE ca.id_curso= :id_curso && ca.id_alumno= :id_alumno && c.estado_clase=4;");
